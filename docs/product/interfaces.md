...
`VersionedRunEventResource` the per-run stream emits. A run whose projection cannot be served is named on this feed as `RUN_PROJECTION_CORRUPT` (`durable-state-corrupt`, `public_run_reference`) and does not end the subscription; the Workbench stands it beside the run list's defective rows, named by its reference, and that run's own event stream still ends with `STREAM_FAILED`. `Last-Event-ID` resumes by same-instant identity
