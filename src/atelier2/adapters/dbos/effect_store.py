...
from atelier2.adapters.dbos.driverless_intents import (
    DriverlessConvergence,
    driverless_convergence,
    driverless_effect_intents,
)
...
def converge_driverless_effect_intents(
    ...
    return tuple(
        logical_key
        for logical_key in driverless_effect_intents(engine, application_version)
...
def _converge_intent(
    ...
    with canonical_write_transaction(engine) as connection:
        ...
        if record is None:
            return False
        convergence = driverless_convergence(connection, record, application_version)
        match convergence:
            case None:
                return False
            case DriverlessConvergence.RECONCILIATION_DOOR:
            ...
            case DriverlessConvergence.ABANDONMENT:
                _abandon_intent(connection, logical_key)
            case DriverlessConvergence.REOPENED_DOOR:
                _reopen_reconciliation(
                    connection,
                    logical_key,
